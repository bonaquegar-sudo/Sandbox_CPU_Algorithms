# app.py - Version 1.2: PDF Export & Custom Algorithms
...
# Custom Algorithm Editor (NUEVA FEATURE)
...
if st.button("RUN SIMULATION", type="primary") and len(st.session_state.processes) > 0:
    ...
    with st.spinner('Processing...'):
        ...
        # --- PDF EXPORT BUTTON (NUEVA FEATURE) ---
        st.divider()
...
def generate_pdf_report(results):
    ...
    for idx, row in df.iterrows():
        ax.barh(row['Process'], row['Duration'], left=row['Start'], height=0.5)
    ...
    if 'ai_report' in results:
        ...
        clean_text = results['ai_report'].replace('#', '').replace('*', '')
